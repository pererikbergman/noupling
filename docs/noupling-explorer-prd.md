...
- **Binary name preserved.** `noupling-cli`'s manifest declares `[[bin]] name = "noupling"` so the produced binary, `cargo install`, Homebrew tap, and release artifacts all keep the name `noupling`. The crates are not published to crates.io (`publish = false` workspace-wide); distribution is Homebrew + GitHub Releases only, so the `[package] name = "noupling"` on the cli crate matters only for the binary name.
