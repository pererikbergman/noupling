...
1. **Scores move for projects with no `layers`.** Layer inference and the actionable coupling-mode fallback, which only the Explorer applied before, now run inside the shared audit pipeline, so `audit`, every report and the CI gate see the same layers and the same Issues. Unconfigured projects usually score higher and may gain Layer Violations (inferred layers never produce `<unlayered>` ones). The text report says `Layers: inferred from path names (…)` when it happened. To turn inference off, set `"infer_layers": false`; declaring your own `layers` also disables it, and a top-level `coupling_mode` keeps your mode while still using inferred layers. Either way, re-check any `--fail-below` threshold.
2. **Re-run `noupling baseline save`.** Baseline files written before 0.9.0 covered only coupling violations (format version 1) and files from unreleased 0.9.0 builds used a different identity (version 2); both load as "nothing baselined" with a one-line warning until re-saved. Baselined Issues are now kept in every report and marked `baselined` instead of being dropped, so `audit --baseline` no longer changes the score; its exit code still depends only on new Issues.
3. **`report.json` / `report.xml` consumers: read `issues`.** The per-kind Issue arrays are gone (breaking); every Issue is in the single top-level `issues` array of Issue cards (documented on the docs site under Report Formats). Filter by `kind`:
...
| `cohesion` | `issues` where `kind == "low_cohesion"` for flagged Packages (`detail`: `cohesion`, `n_children`, `internal_deps`); Containers and cohesive Packages are no longer listed |
   ...
   Two enumerations changed with the move: `zone` is now `pain` / `uselessness` (was `zone_of_pain` / `zone_of_uselessness`) and `flag_type` is `fused_sibling` / `trapped_child` (was `FusedSibling` / `TrappedChild`). The Metric arrays `hotspots`, `abstractness`, `instability` and `directory_tree` are unchanged (`directory_tree` entries gain `kind`: `Container` or `Package`, which the removed `cohesion` array used to carry); per-directory D is recomputable from `abstractness` + `instability`, and cohesion for Packages that are *not* flagged is no longer reported. The header counts `critical_violations`, `total_circular`, `total_coupling` now count violations as `issues` does (a ring's hop edges belong to its Cycle, #358), so `issues.filter(kind == "coupling_violation").length == total_coupling`. In XML the `<circular-dependencies>`, `<coupling-violations>`, `<gravity-wells>` and `<red-flags>` elements are replaced by `<issues>`; nested payloads (a Cycle's `hops`) appear as child elements of `<detail>`.
   The Metric arrays `hotspots`, `abstractness`, `instability` and `directory_tree` are unchanged (`directory_tree` entries gain `kind`: `Container` or `Package`); per-directory D is recomputable from `abstractness` + `instability`, and cohesion for unflagged Packages is no longer reported. The header counts `critical_violations`, `total_circular`, `total_coupling` count violations as `issues` does (a ring's hop edges belong to its Cycle), so `issues.filter(kind == "coupling_violation").length == total_coupling`. In XML the `<circular-dependencies>`, `<coupling-violations>`, `<gravity-wells>` and `<red-flags>` elements are replaced by `<issues>`; nested payloads (a Cycle's `hops`) appear as child elements of `<detail>`.
4. **Explorer Data Contract is `format_version` 2.** It embeds the same `issues` cards (each with `participants`); `gravity_wells` and `red_flags` are gone from the contract, `cycles` and `violations` remain as canvas geometry. Anything reading the embedded `noupling-data` JSON must switch to `issues`.
5. **Sonar rule ids changed.** Every Issue is exported as `noupling:<kind>` (nine rule ids, severity from the band); the old `noupling:coupling` / `noupling:circular-dependency` ids are gone.

### Added

- **`Issue` in core + Issue cards in the text report** (#340, epic #338): `noupling-core` gains an `Issue` enum with one variant per Issue kind (Coupling Violation, Cycle, Rule Violation, Layer Violation, Gravity Well, Red Flag, Stability Violation, Zone Flag, Low Cohesion) and an `AuditResult::issues()` accessor that yields every Issue in canonical order (severity band descending, then kind, then subject path). Each Issue carries the Issue-card header from `CONTEXT.md`: kind, severity band (critical / high / medium / low, assigned once in core), subject, a one-sentence reason with this instance's numbers, and a one-sentence recommendation. A mutually-dependent pair is one Cycle and never also its constituent Coupling Violations. The text report (`noupling audit`) now renders its Issue sections as these cards under one `Issues (N)` heading; the headline numbers and Metric sections are unchanged.

- **Score impact on every Issue** (#342): `Issue::score_impact()` reports the points an Issue takes off the project score, assigned in core by the same pass that computes the score, so the sum over all Issues equals `100 − score` (when the score is clamped at 0, impacts are scaled to sum to the 100 points lost). A Cycle's impact includes the ring hops folded into it; Rule / Layer / Stability Violations, Gravity Wells, Red Flags, Zone Flags and Low Cohesion report zero. `CouplingViolation` gains a `score_impact` field. The text report prints `Points lost: 4.7 (Coupling Violation 0.9, Cycle 3.8)` under the Issues heading and a `Score impact` line on every card. The scoring formula is unchanged.

- **Baseline over every Issue kind** (#343): `baseline save` fingerprints each Issue by kind and subject (all nine kinds, file format version 2). `--baseline` is now a pipeline input accepted by both `audit` and `report`; matching Issues stay in `issues()` with `Issue.baselined = true` so formats can mark or count them ("Issues (7): 0 new, 7 baselined; …"). `audit --baseline` prints new / baselined / resolved counts and fails only on new Issues. `Issue` is now a struct (`baselined` + `detail: IssueDetail`) and exposes `fingerprint()`; the coupling detector picks a deterministic representative edge per directory pair so fingerprints are stable across scans. New `report --format text` writes `.noupling/report.txt`.

- **JSON `issues` array; xml and sonar derive from it** (#344, first half of ADR 0002): `report.json` gains a top-level `issues` array with one Issue card per Issue (kind, kind_name, severity, subject, reason, recommendation, score_impact, baselined, fingerprint, per-kind `detail`), serialised once in core as `IssueCard` so every consumer embeds the same shape. The XML report carries the same cards as `<issues>/<issue>`; the Sonar report now emits one generic issue per Issue for all nine kinds (`ruleId` `noupling:<kind>`, band → CRITICAL / MAJOR / MINOR / INFO), where it previously covered only coupling and cycles. Shape documented in the docs site under Report Formats.

- **html + md render Issue cards for all nine kinds** (#346): both directory-tree reports now render an `Issues` section from `issues()` — the root page carries a kind-count summary and every card in canonical order, each directory page only the Issues anchored under it (`Issue::anchor_dir()`: the deepest directory containing the subject). Cards show band, subject, reason, recommendation and score impact; baselined Issues are marked. The old Circular Dependencies / Coupling Violations / Stability Violations sections are replaced by the cards; Metric sections (abstractness, instability, distance, coupling metrics) stay.

- **dashboard, pr and briefing carry every Issue kind** (#347): the dashboard gains a nine-tile "issues by kind" row (zero counts included) and a full Issue table built from the embedded Issue cards; the PR comment prints an `Issues` count, a `By kind:` line naming all nine kinds with counts, and the top five Issues by band with their recommendation (capped so the comment stays short); the briefing ranks its top ten by score impact then band, lists per-kind counts in its summary, and its "Approach" text is the Issue's recommendation verbatim. Each has an exhaustive match on the Issue payload.
...
- **Format-class rule asserted** (#350): `tests/issue_coverage.rs` no longer pins a coverage matrix; it asserts the `CONTEXT.md` format-class rule against each format's data — every Issue-listing format carries all nine kinds, every graph format accents all five edge-shaped kinds, the trend format has a recorded series per kind. Graph formats now draw one accented edge per kind on a pair (a pair that is both a Coupling and a Stability Violation gets both accents) instead of letting one kind hide the other; an edge's `|N|` label counts Issues of that kind on the pair. Mermaid and DOT key nodes by full directory path (label = basename), so two `models/` directories no longer collapse into one node, and node colour follows every accented edge, not just coupling. The removal of the per-kind JSON arrays is the breaking change at the top of this section.

- **`infer_layers` setting** (#356): `"infer_layers": false` in `.noupling/settings.json` turns layer inference off, so an unlayered project audits as-is in strict mode. Default true; written by `noupling init`.

### Fixed

- **Explorer: gravity-well focus shows the well and the modules pulling on it** (#333): focusing a Gravity Well scopes to the well plus every contributing module, with the contributing edges highlighted, instead of one lonely card (participants come from the Issue card since #345; this adds the smoke spec and `data-edge` / `data-accent` hooks on LSM edges).
- **`Violations:` counts agree with the Issue cards** (#358): every headline count — the text report, `pr`, `briefing`, `html`, `dashboard`, `bundle`, `trend`, the JSON `total_coupling` / `total_circular` header, per-directory counts and Violation Age — now counts violations the way `issues()` does: a ring is one Cycle, its hop edges are not counted again as Coupling Violations (`AuditResult::violation_count()` / `issue_violations()`). The raw `violations` list is unchanged, so the score and TRI are unaffected.
- **Review follow-ups** (second pass over #367): an SCC with several elementary cycles now reports the same ring on every scan (the DFS starts from the smallest member); Violation Age keys on the directory pair like the baseline does; diff-scoped scans record the snapshot's whole-project trend point (captured before the diff filter) so a CI project that only runs `scan --diff-base` keeps its history, while `--module` runs record nothing; Sonar pins Gravity Wells to a file too and prefers a direct child file over one in a nested package; graph formats no longer draw an edge between two directories that share a basename as a self-loop; the dashboard's type bar and the bundle's ring overlay take their colours from the shared palette; baseline file version bumps to 3 (files written by unreleased 0.9.0 builds are treated as legacy).
...
- **`critical_severity` is honoured again** (#368): the severity band of Coupling Violations and Cycles steps to *critical* at `thresholds.critical_severity` (high and medium at 0.4× and 0.2× of it, so the default 0.5 keeps today's 0.2 / 0.1 steps), and the JSON `critical_violations` count uses the same threshold. The audit result records the threshold it was judged under; the band is assigned once and every format shows it.
